...
/**
 * GitHub'ın son "push" aktivitesini `git log --oneline` estetiğinde,
 * terminal stilinde akan bir akışla gösterir. Yüzdelik/radar yerine
 * karakteristik, geliştirici-dostu bir imza. Veri ISR ile canlı gelir.
 */
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-signal/20 bg-[#070b0f]/90 shadow-[0_0_50px_rgba(0,0,0,0.4)] ring-1 ring-white/5 backdrop-blur-xl">
      <div className="flex items-center gap-2 border-b border-white/10 bg-white/[0.03] px-4 py-3">
        ...
        <span className="ml-2 font-mono text-xs text-cyan-200/70">git log --oneline · son push aktivitesi</span>
      </div>
      ...
      <div className="space-y-1 px-4 py-4 font-mono text-[0.8rem] leading-relaxed sm:text-sm">
        {entries.map((entry, index) => (
          ...
          >
            <span className="text-amber-400/90">{entry.hash}</span>
        ...
        <div className="flex items-center gap-2 px-1 pt-2 text-cyan-200/70">
          ...
          <span className="inline-block h-4 w-2 animate-pulse bg-cyan-300/70" aria-hidden />
        </div>
      </div>
    </div>
  );
